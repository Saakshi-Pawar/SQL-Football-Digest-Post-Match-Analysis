...
select `group`, avg(exp_goal_difference_per_90) from sports_1 group by `group` order by avg(exp_goal_difference_per_90);
...
select `group`, avg(avg_age) as avg_group_age
from sports_2 inner join sports_1
on sports_2.team=sports_1.team
group by `group`
